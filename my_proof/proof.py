...
class Proof:
            ...
            if os.path.splitext(input_file)[1].lower() == '.json':
                with open(input_file, 'r') as f:
                    ...
                    if schema_type == 'instagram-meta-export.json':
                        self._process_instagram_data(input_data, schema_matches, google_user, errors)
                    else:
                        self._process_google_data(input_data, schema_matches, google_user, errors)
        ...
        try:
            ...
            if google_user:
                ...
                if contributor_email != google_user.email:
                    ...
                    logging.error(f"Contributor email {contributor_email} does not match Google email {google_user.email}")
            ...
            authenticity_score = self._calculate_instagram_authenticity_score(instagram_data, google_user)
            
            # Calculate uniqueness score based on account activity and engagement
            uniqueness_score = self._calculate_instagram_uniqueness_score(instagram_data)
    ...
    def _calculate_instagram_quality_score(self, instagram_data: InstagramContribution) -> float:
        ...
        has_activities = (
            len(instagram_data.data.activities.posts_created) > 0 or
            len(instagram_data.data.activities.likes_given) > 0 or
            len(instagram_data.data.activities.comments_made) > 0 or
            len(instagram_data.data.activities.following_list) > 0
        )
        if has_activities:
            score += 0.1
    ...
    def _calculate_instagram_authenticity_score(self, instagram_data: InstagramContribution, google_user) -> float:
        ...
        if metrics.total_interactions == (metrics.likes_given_count + metrics.comments_count):
            score += 0.1
        if metrics.account_age_days > 0:
            score += 0.1
        
        # Source verification (10%)
        if instagram_data.data.source_type == "meta_export" and instagram_data.data.extraction_method == "google_drive_api":
            score += 0.1
        
        return min(score, 1.0)
    
    def _calculate_instagram_uniqueness_score(self, instagram_data: InstagramContribution) -> float:
        """Calculate uniqueness score based on account activity and engagement."""
        score = 0.0
        metrics = instagram_data.data.metrics
